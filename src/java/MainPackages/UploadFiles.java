...
public class UploadFiles extends HttpServlet 
{
   ...
   // public String AUTHORIZATION_URL;
    ...
    public String UPLOAD_DIR_NAME;
        ...
        private int document_max_file_size = 5000 * 1024;//bytes for document
        private String[] document_acceptable_content_types=new String[]{"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet","application/vnd.ms-excel","application/pdf","application/vnd.openxmlformats-officedocument.wordprocessingml.document","application/msword"};//for document
        private String[] document_acceptable_extensions=new String[]{"xlsx","xls","pdf","docx","doc"};//for document
    /**
     *
     * @param Configuration
                ...
                if(!session_key.isEmpty())
                {
                        try 
                        {
                           ...
                           // System.out.println(ReturnDocumentIs);
                            ...
                            {
                                //do the upload
                               Document return_document_is= DoTheUpload(request,response,email_address_is);
    ...
    private Document DoTheUpload(HttpServletRequest request, HttpServletResponse response,String email_address)
    {
                        ...
                        String file_extension=null;
                                ...
                                for(FileItem item : multiparts)
                                {
                                        if(!item.isFormField())
                                        {
                                            ...
                                            //make sharable link
                                            String sharable_link=FILE_SERVER_BASE_URL+email_address_md5+"/"+time_stamp_is+"/"+file_name;
                                                ...
                                                else if(file_size<=document_max_file_size && Arrays.asList(document_acceptable_content_types).contains(file_type) && Arrays.asList(document_acceptable_extensions).contains(file_extension) )//check conditions for documents
                                                {
                                                    item.write( new File(TimestampDir + File.separator + file_name));//save file
                                                ...
                                                else
                                                {
                                                        ...
                                                        String document_max_size_mbs=Float.toString(document_max_file_size/(1000 * 1024))+" MB";
                                                        
                                                        return_document_is.append("check",false );
                                                        return_document_is.append("message", "File must be of extensions "+Arrays.toString(image_acceptable_extensions)+" or "+Arrays.toString(document_acceptable_extensions)+" of sizes "+image_max_size_mbs+" or "+document_max_size_mbs+" respectively" );
