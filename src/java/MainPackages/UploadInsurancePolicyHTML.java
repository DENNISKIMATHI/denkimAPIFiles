 ...
 * @author mutemi
 */
public class UploadInsurancePolicyHTML extends HttpServlet 
{
   ...
   // public String AUTHORIZATION_URL;
    ...
    public String UPLOAD_DIR_NAME;
        ...
        private int html_max_file_size = 1000 * 1024;//bytes for image
        private String[] html_acceptable_content_types=new String[]{"text/html"};//for image
        private String[] html_acceptable_extensions=new String[]{"html"};//for html
                        ...
                        String file_extension=null;
                                ...
                                for(FileItem item : multiparts)
                                {
                                        if(!item.isFormField())
                                        {
                                            ...
                                            String is_policy_html="policy_html";
                                            //make md5 email
                                            MakeMD5String md5_object=new MakeMD5String(is_policy_html);
                                            String is_policy_html_md5=md5_object.Md5StringIs();
            
                                            String UserDir=UPLOAD_DIR_NAME+"\\"+is_policy_html_md5;
                                            ...
                                            //make sharable link
                                            String sharable_link=FILE_SERVER_BASE_URL+is_policy_html_md5+"/"+time_stamp_is+"/"+file_name;
                                            
                                                //images
                                                if(file_size<=html_max_file_size && Arrays.asList(html_acceptable_content_types).contains(file_type) && Arrays.asList(html_acceptable_extensions).contains(file_extension) )//check conditions for images
                                                {
                                                ...
                                                {
                                                        String html_max_size_mbs=Float.toString(html_max_file_size/(1000 * 1024))+" MB";
                                                       
                                                        
                                                        return_document_is.append("check",false );
                                                        return_document_is.append("message", "File must be of extensions "+Arrays.toString(html_acceptable_extensions)+" of sizes "+html_max_size_mbs+file_type );
